...
class tableFrontend {
  ...
  public function action() {
    ...
    if (!LEPTON_2) {
      ...
      elseif (is_registered_droplet_js('flex_table', PAGE_ID)) {
  ...
  /**
   * Stellt die Daten der Tabelle zusammen und uebergibt sie an das Template
   * table.htt
   *
  ...
  public function showTable() {
    ...
    $where = array(
      dbFlexTable::field_name => $this->params[self::param_name]
    );
    $table = array();
    ...
    $SQL = sprintf("SELECT * FROM %s WHERE %s='%s' ORDER BY FIND_IN_SET(%s, '%s')", $dbFlexTableDefinition->getTableName(), dbFlexTableDefinition::field_table_id, $table_id, dbFlexTableDefinition::field_id, $table[dbFlexTable::field_definitions]);
    $definitions = array();
    ...
    foreach ($definitions as $def) {
      if ($def[dbFlexTableDefinition::field_table_cell] == dbFlexTableDefinition::cell_true) {
        // filter anlegen
        if (isset($_REQUEST[sprintf('%s_%s', self::request_filter, $def[dbFlexTableDefinition::field_id])]) && $_REQUEST[sprintf('%s_%s', self::request_filter, $def[dbFlexTableDefinition::field_id])] != self::filter_none) {
        ...
        if (!empty($field) && (($field != dbFlexTableCell::field_text) && ($field != dbFlexTableCell::field_html))) {
          $SQL = sprintf("SELECT %s AS filter FROM %s WHERE %s='%s' AND %s='%s' GROUP BY %s ORDER BY %s ASC", $field, $dbFlexTableCell->getTableName(), dbFlexTableCell::field_table_id, $table_id, dbFlexTableCell::field_definition_id, $def[dbFlexTableDefinition::field_id], $field, $field);
          $result = array();
          ...
          $filter_array = array(
            self::filter_none => array(
              'value' => self::filter_none,
              'text' => ft_filter_none,
              'selected' => ($act_filter == self::filter_none) ? 1 : 0
            ),
            self::filter_asc => array(
              'value' => self::filter_asc,
              'text' => ft_filter_asc,
              'selected' => ($act_filter == self::filter_asc) ? 1 : 0
            ),
            self::filter_desc => array(
              'value' => self::filter_desc,
              'text' => ft_filter_desc,
              'selected' => ($act_filter == self::filter_desc) ? 1 : 0
            )
          );
          ...
          $filter_array = array();
          $filter_array[self::filter_none] = array(
            'value' => self::filter_none,
            'text' => ft_filter_none,
            'selected' => 0
          );
        ...
        $def_array[$def[dbFlexTableDefinition::field_name]] = array(
          ...
          'link' => sprintf('%s%s%s=', $this->page_link, (defined('LEPTON_VERSION') && isset($_GET['leptoken'])) ? sprintf('?leptoken=%s&amp;', $_GET['leptoken']) : '?', sprintf('%s_%s', self::request_filter, $def[dbFlexTableDefinition::field_id]))
        );
      }
    }
    ...
    if ($active_filter_id == -1) {
      $SQL = sprintf("SELECT * FROM %s WHERE %s='%s' AND (%s='%s' OR %s='%s') ORDER BY %s ASC, FIND_IN_SET(%s, '%s')", $dbFlexTableCell->getTableName(), dbFlexTableCell::field_table_id, $table_id, dbFlexTableCell::field_table_cell, dbFlexTableDefinition::cell_true, dbFlexTableCell::field_definition_name, 'permalink', dbFlexTableCell::field_row_id, dbFlexTableCell::field_definition_id, $table[dbFlexTable::field_definitions]);
    }
    else {
      // Filter verwenden
      $where = array(
        dbFlexTableDefinition::field_id => $active_filter_id
      );
      $def = array();
      ...
      if ($active_filter == self::filter_asc || $active_filter == self::filter_desc) {
        $SQL = sprintf("SELECT %s FROM %s WHERE %s='%s' ORDER BY %s %s", dbFlexTableCell::field_row_id, $dbFlexTableCell->getTableName(), dbFlexTableCell::field_definition_id, $def[dbFlexTableDefinition::field_id], $field, $active_filter);
        $order = array();
        ...
        $SQL = sprintf("SELECT * FROM %s WHERE %s='%s' AND (%s='%s' OR %s='%s') ORDER BY FIND_IN_SET(%s, '%s'), FIND_IN_SET(%s, '%s')", $dbFlexTableCell->getTableName(), dbFlexTableCell::field_table_id, $table_id, dbFlexTableCell::field_table_cell, dbFlexTableDefinition::cell_true, dbFlexTableCell::field_definition_name, 'permalink', dbFlexTableCell::field_row_id, $row_order, dbFlexTableCell::field_definition_id, $table[dbFlexTable::field_definitions]);
      }
      else {
        // nach Gruppen filtern
        $SQL = sprintf("SELECT %s FROM %s WHERE %s='%s' AND %s='%s' AND %s LIKE '%s' ORDER BY %s ASC", dbFlexTableCell::field_row_id, $dbFlexTableCell->getTableName(), dbFlexTableCell::field_table_id, $table_id, dbFlexTableCell::field_definition_name, $def[dbFlexTableDefinition::field_name], $field, $_REQUEST[sprintf('%s_%s', self::request_filter, $active_filter_id)], dbFlexTableCell::field_row_id);
        $order = array();
        if (!$dbFlexTableCell->sqlExec($SQL, $order)) {
          $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $dbFlexTableCell->getError()));
          return false;
        ...
        $SQL = sprintf("SELECT * FROM %s WHERE %s='%s' AND (%s='%s' OR %s='%s') AND %s IN (%s) ORDER BY %s ASC, FIND_IN_SET(%s, '%s')", $dbFlexTableCell->getTableName(), dbFlexTableCell::field_table_id, $table_id, dbFlexTableCell::field_table_cell, dbFlexTableDefinition::cell_true, dbFlexTableCell::field_definition_name, 'permalink', dbFlexTableCell::field_row_id, $row_order, dbFlexTableCell::field_row_id, dbFlexTableCell::field_definition_id, $table[dbFlexTable::field_definitions]);
      }
    } // Filter
    $rows = array();
    ...
    foreach ($rows as $row) {
      ...
      if (($row[dbFlexTableCell::field_definition_type] == dbFlexTableDefinition::type_media_link) && (!empty($value))) {
    ...
    $data = array(
      ...
      'anchor' => array(
        'detail' => $dbFlexTableCfg->getValue(dbFlexTableCfg::cfgAnchorDetail),
  ...
  /**
   * Show the details of a row record - also this items which are not shown in
   * the table
   * showDetail() can be called from the table in "table" mode or from showID()
   * in "detail" mode
   *
   * @param INT $row_id
  ...
  public function showDetail($row_id = -1) {
    ...
    $SQL = sprintf("SELECT * FROM %s WHERE %s='%s'", $dbFlexTable->getTableName(), dbFlexTable::field_id, $table_id);
    $table = array();
    ...
    $SQL = sprintf("SELECT * FROM %s WHERE %s='%s'ORDER BY FIND_IN_SET(%s, '%s')", $dbFlexTableCell->getTableName(), dbFlexTableCell::field_row_id, $row_id, dbFlexTableCell::field_definition_id, $table[dbFlexTable::field_definitions]);
    $items = array();
    ...
    foreach ($items as $item) {
      $where = array(
        dbFlexTableDefinition::field_id => $item[dbFlexTableCell::field_definition_id]
      );
      $definition = array();
      ...
      if (($item[dbFlexTableCell::field_definition_type] == dbFlexTableDefinition::type_media_link) && (!empty($value))) {
        $ext = strtolower(pathinfo($this->media_path . $value, PATHINFO_EXTENSION));
        $basename = pathinfo($this->media_path . $value, PATHINFO_FILENAME);
        $media_type = $ext;
        $width = 0;
  ...
  public function showID() {
    ...
    if ($this->params[self::param_show_last] > 0) {
      ...
      if (!empty($this->params[self::param_name])) {
        ...
        foreach ($tabs as $tab) {
          $tab = trim($tab);
          $SQL = sprintf("SELECT %s FROM %s WHERE %s='%s'", dbFlexTable::field_id, $dbFlexTable->getTableName(), dbFlexTable::field_name, $tab);
          $table = array();
          if (!$dbFlexTable->sqlExec($SQL, $table)) {
            $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $dbFlexTable->getError()));
            return false;
      ...
      $SQL = sprintf("SELECT * FROM %s %s ORDER BY %s DESC LIMIT %s", $dbFlexTableRow->getTableName(), $tab_str, dbFlexTableRow::field_timestamp, $limit);
      $result = array();
      ...
      foreach ($result as $tab)
        $ids[] = $tab[dbFlexTableRow::field_id];
    }
    else {
